...
CREATE POLICY merchants_own_row ON merchants
  USING (id::TEXT = auth.uid() OR is_admin())
  WITH CHECK (id::TEXT = auth.uid() OR is_admin());

-- products : merchant owner or admin
CREATE POLICY products_owner ON products
  USING (merchant_id::TEXT = auth.uid() OR is_admin())
  WITH CHECK (merchant_id::TEXT = auth.uid() OR is_admin());
...
CREATE POLICY orders_merchant_or_customer ON orders
  USING (merchant_id::TEXT = auth.uid() OR customer_id::TEXT = auth.uid() OR is_admin())
  WITH CHECK (merchant_id::TEXT = auth.uid() OR customer_id::TEXT = auth.uid() OR is_admin());

-- order_items : accessible via order ownership
CREATE POLICY order_items_via_order ON order_items
  USING (EXISTS (SELECT 1 FROM orders o WHERE o.id = order_items.order_id AND (o.merchant_id::TEXT = auth.uid() OR o.customer_id::TEXT = auth.uid() OR is_admin())))
  WITH CHECK (EXISTS (SELECT 1 FROM orders o WHERE o.id = order_items.order_id AND (o.merchant_id::TEXT = auth.uid() OR o.customer_id::TEXT = auth.uid() OR is_admin())));
...
CREATE POLICY reviews_manage ON reviews
  FOR ALL USING (buyer_id::TEXT = auth.uid() OR EXISTS (SELECT 1 FROM products p WHERE p.id = reviews.product_id AND p.merchant_id::TEXT = auth.uid()) OR is_admin())
  WITH CHECK (buyer_id::TEXT = auth.uid() OR EXISTS (SELECT 1 FROM products p WHERE p.id = reviews.product_id AND p.merchant_id::TEXT = auth.uid()) OR is_admin());
